...
The script will symlink all required Apache Ignite Hadoop Accelerator JARs directly to Hadoop installation directory. In addition the script can replace the content of `core-site.xml` and `mapred-site.xml` and will let you use Ignite File System as a default one for Hadoop and enable Ignite `MapReduce` job tracker.
[block:callout]
{
  "type": "warning",
