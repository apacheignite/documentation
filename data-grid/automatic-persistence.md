...
The utility generates the following artifacts:
  ...
  * XML configuration for `Query entities` and `JDBC types`.
...
After you exit from the wizard, you should
 ...
 2. Copy XML declaration of `QueryEntity` beans and `CacheJdbcPojoStoreFactory` bean to your Ignite XML configuration file under appropriate `CacheConfiguration` root.
...
Another example is `org.apache.ignite.examples.binary.datagrid.store.auto.CacheBinaryAutoStoreExample` the main difference is that in this example data source declared as bean in XML.
[block:api-header]
{
  "type": "basic",
  "title": "Demo"
