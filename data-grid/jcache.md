...
{
  "type": "warning",
  "body": "If batch operations (such as `IgniteCache#putAll`, `IgniteCache#invokeAll`, etc.) are performed in parallel, then keys should be ordered in the same way to avoid deadlock. It's recommended to use `TreeMap` instead of `HashMap` to guarantee consistent ordering. Note that this is true for both `ATOMIC` and `TRANSACTIONAL` caches.",
  "title": "Deadlock"
}
[/block]
