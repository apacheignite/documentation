...
All the concepts that are discussed as a part of annotation based configuration above are valid for query entities based approach. Event more, types which fields are configured with `@QuerySqlField` and registered with `CacheConfiguration.setIndexedTypes` method are internally converted to query entities.
