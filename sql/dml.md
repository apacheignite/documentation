...
One possible approach is to execute `EXPLAIN` for the `SELECT`query that is automatically generated (`UPDATE`, `DELETE`) or used (`INSERT`, `MERGE`) by DML statements. This will give you an insight on the indexes that are used while a DML operation is executed.
[block:api-header]
{
  "type": "basic",
  "title": "Example"
