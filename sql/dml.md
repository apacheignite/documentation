...
[/block]
##Batch updates are not supported by JDBC driver
As stated in [JDBC driver doc](doc:jdbc-driver-1.8), batch updates are yet to come. Still you of course can form a (_very_) long query for **INSERT** and **MERGE** in your code, provide needed arguments, and rock with inserting tens or hundreds of items per query anytime your heart desires.

##Scope of subqueries in WHERE
Although subqueries for **INSERT** and **MERGE** may be distributed, as well as **SELECT**s generated by **UPDATE** and **DELETE**, if a **WHERE** clause of such select contains a subquery, **it will not be distributed**. Example:
[block:code]
{
  "codes": [
    {
...
Ignite performs DML operations in a very different way than a conventional RDBMS does. Therefore using H2's **EXPLAIN** output with DML operations as it's done with **SELECT**s does not look like a good solution. One possible approach is just to **EXPLAIN** the **SELECT**s generated/used by DML operations (because in many cases there's one) - this will give insight on which indexes are used while executing DML operations. Probably this will be implemented in near future.