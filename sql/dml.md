...
To execute DML statements in Java, you need to use the same API that is used for `SELECT` queries -  [`SqlFieldsQuery` API](doc:sql-queries#section-sqlfieldsqueries).
...
`SELECT` queries used in `INSERT` and `MERGE` statements as well as `SELECT` queries automatically generated by `UPDATE` and `DELETE` operations will be distributed and executed in either [collocated or non-collocated distributed modes](https://apacheignite.readme.io/docs/sql-queries#distributed-joins) if needed. 

However, if there is a subquery that is executed as a part of  `WHERE` clause then it will not be executed in non-collocated distributed mode. The subquery will be executed in the collocated mode over the local data set all the times.

For instance, if there is a query like that
[block:code]
{
  "codes": [
    {
...
[/block]
then DML engine will generate the `SELECT` query in order to get a list of entries that will be deleted. The query will be distributed and executed across the cluster and will look like the one below
[block:code]
{
  "codes": [
    {
